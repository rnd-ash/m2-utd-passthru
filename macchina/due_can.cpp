...
uint32_t CANRaw::init(uint32_t ul_baudrate)
{
	uint32_t ul_flag;
	uint32_t ul_tick;
	//PCCOMM::logToSerial("Init buffers...");
    initializeBuffers();
	//PCCOMM::logToSerial("Init buffers completed");
...
//arduino 1.5.2 doesn't init canbus so make sure to do it here. 
	...
	//PCCOMM::logToSerial("PIO configure completed");
#endif
	
	//PCCOMM::logToSerial("PMC enable...");
	if (m_pCan == CAN0) pmc_enable_periph_clk(ID_CAN0);
	if (m_pCan == CAN1) pmc_enable_periph_clk(ID_CAN1);
	//PCCOMM::logToSerial("PMC enable completed");
	if (enablePin != 255) {
		//PCCOMM::logToSerial("Pin mode set...");
		pinMode(enablePin, OUTPUT);
		digitalWrite(enablePin, HIGH);
		//PCCOMM::logToSerial("Pin mode set completed");
	}

	/* Initialize the baudrate for CAN module. */
	//PCCOMM::logToSerial("Set baudrate...");
	ul_flag = set_baudrate(ul_baudrate);
	//PCCOMM::logToSerial("Set baudrate completed");
	if (ul_flag == 0) {
		return 0;
	}

	/* Reset the CAN eight message mailbox. */
	//PCCOMM::logToSerial("Reset mailboxes...");
	reset_all_mailbox();

	//Also disable all interrupts by default
	//PCCOMM::logToSerial("Disable interrupt...");
	disable_interrupt(CAN_DISABLE_ALL_INTERRUPT_MASK);

	//By default use one mailbox for TX 
	//PCCOMM::logToSerial("Set TX Mailbox...");
	setNumTXBoxes(numTXBoxes);

	/* Enable the CAN controller. */
	//PCCOMM::logToSerial("Enable CAN...");
	enable();

	/* Wait until the CAN is synchronized with the bus activity. */
	ul_flag = 0;
	ul_tick = 0;
	//PCCOMM::logToSerial("Syncronizing CAN...");
	...
	//PCCOMM::logToSerial("Setting priority...");
